...
const update = async ({ body, auth }, { sendErrorResponse, sendSuccessResponse }) => {
    ...
    const { id, tid, type, content, origin, title, desc_txt, preview_img, is_show, download = true } = body;
    if (!id) return sendErrorResponse({ msg: '更新失败，未查询到ID' });
    if (type && !typeList.includes(type)) return sendErrorResponse({ msg: '更新失败，错误的类型' });
    const sets = {
        tid, type, content, origin, title, desc_txt, preview_img, is_show, update_date: dateformat()
    };
    try {
        // 更新数据，返回更新条数
